...
public class InputUtils extends MaskFormatter
{
	...
	public static LocalTime stringToTime(String value) {
		try {
			...
			return LocalTime.from(format.parse(value));
		}
		catch (Exception e) {
	...
	public static LocalDate[] stringToDate(DateTimeFormatter format, String separator,
			String value) {
		try {
	...
	public static void useDateInput(JFormattedTextField txt, String pattern, boolean between,
			String separator, ValueCallback callback,
			InputValidationListener inputValidationListener) {
		try {
			String format = datePatternToInputFormat(pattern, "#");
			DateInputFormat mask = new DateInputFormat(
					between ? format + separator + format : format, between, separator, pattern,
	...
	public static void changeDateFormatted(JFormattedTextField txt, String pattern, boolean between,
			String separator, InputValidationListener inputValidationListener) {
		try {
			String format = datePatternToInputFormat(pattern, "#");
			DateInputFormat mask = new DateInputFormat(
					between ? format + separator + format : format, between, separator, pattern,
	...
	private static OldEditorProperty initEditor(JFormattedTextField txt, MaskFormatter format,
			ValueCallback callback) {
		...
		txt.putClientProperty(FlatClientProperties.TEXT_FIELD_CLEAR_CALLBACK,
				(Consumer<Object>) o -> {
					txt.setValue(null);
	...
	private static void putPropertyChange(JFormattedTextField txt,
	...
	private static class DateInputFormat extends MaskFormatter
	{
		...
		public DateInputFormat(String mark, boolean between, String separator, String pattern,
				InputValidationListener inputValidationListener) throws ParseException {
			super(mark);
	...
	private static class OldEditorProperty
	{
		...
		protected static OldEditorProperty getFromOldEditor(JFormattedTextField editor) {
			...
			oldEditorProperty.oldTrailingComponent = FlatClientProperties.clientProperty(editor,
					FlatClientProperties.TEXT_FIELD_TRAILING_COMPONENT, null, Component.class);
			oldEditorProperty.isShowClearButton = FlatClientProperties.clientPropertyBoolean(editor,
					FlatClientProperties.TEXT_FIELD_SHOW_CLEAR_BUTTON, false);
			oldEditorProperty.clearButtonCallback = FlatClientProperties.clientProperty(editor,
					FlatClientProperties.TEXT_FIELD_CLEAR_CALLBACK, null, Consumer.class);
			oldEditorProperty.outline = FlatClientProperties.clientProperty(editor,
					FlatClientProperties.OUTLINE, null, String.class);
		...
		protected void removeFromEditor(JFormattedTextField editor) {
			...
			editor.putClientProperty(FlatClientProperties.TEXT_FIELD_TRAILING_COMPONENT,
					oldTrailingComponent);
			editor.putClientProperty(FlatClientProperties.TEXT_FIELD_SHOW_CLEAR_BUTTON,
					isShowClearButton);
			editor.putClientProperty(FlatClientProperties.TEXT_FIELD_SHOW_CLEAR_BUTTON,
					clearButtonCallback);
